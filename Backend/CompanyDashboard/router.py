...
async def get_dashboard_stats(email: str, db: Session = Depends(get_db)):
    ...
    founder_count = (
        db.query(OrgMember)
        .filter(OrgMember.org_id == org.id, OrgMember.member_type == "Founder")
        .count()
    )
    employee_count = (
        db.query(OrgMember)
        .filter(OrgMember.org_id == org.id, OrgMember.member_type == "Employee")
        .count()
    )
    customer_count = (
        db.query(OrgMember)
        .filter(OrgMember.org_id == org.id, OrgMember.member_type == "Customer")
        .count()
    )
